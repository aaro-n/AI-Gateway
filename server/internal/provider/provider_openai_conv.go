...
func (m *OpenAIProvider) ExecuteGeminiRequest(c *gin.Context, pm *model.ProviderModel, usage *Usage) error {
	c.JSON(http.StatusBadRequest, gin.H{"error": "Gemini native requests not supported by OpenAI provider"})
	return fmt.Errorf("Gemini native requests not supported by OpenAI provider")
}
